...
class UpdateSavedCityUseCase @Inject constructor(private val mySearchCityRepositoryImpl: SearchCityRepositoryImpl) {
    suspend operator fun invoke(selectedCity: SelectedCity) = withContext(Dispatchers.IO){
        mySearchCityRepositoryImpl.updateCity(selectedCity)
    }
}